...
function StudentMarksEntry() {
    ...
    useEffect(() => {
        const fetchSchoolDataFromFirebase = async (school) => {
            const response = await axios.get(`https://marksentry-bcdd1-default-rtdb.firebaseio.com/schools/${school}/Class-4.json`);
    ...
    const saveDataToDatabase = () => {
        ...
        axios
            .put(`https://marksentry-bcdd1-default-rtdb.firebaseio.com/schools/${selectedSchool}/Class-4.json`, students)
            .then(() => {
                                        ...
                                        student[subject].map((value, subIndex) => (
                                            <td key={`${subject}-${subIndex}`}>
                                                {subIndex < 7 ? (
                                                    ...
                                                    disabled={selectedSchool === 'Talaricheruvu'}
                                                    />
                                                ) : (
                                                    <span>{value}</span>
                                                )}
