...
function StudentMarksEntry() {
    ...
    useEffect(() => {
        const fetchSchoolData = async (school) => {
            const response = await fetch('studentsData3.json');
    ...
    const saveToDatabase = async () => {
        ...
        try {
            for (const student of students) {
                ...
                const response = await axios.get(`https://marksentry-bcdd1-default-rtdb.firebaseio.com/FA1MARKS/Class-3/${schoolName}/${penNumber}.json`);

                if (response.data) {
                    // Student data already exists, update the record
                    await axios.put(`https://marksentry-bcdd1-default-rtdb.firebaseio.com/FA1MARKS/Class-3/${schoolName}/${penNumber}.json`, student);
                } else {
                    // Student data does not exist, create a new record
                    await axios.post(`https://marksentry-bcdd1-default-rtdb.firebaseio.com/FA1MARKS/Class-3/${schoolName}.json`, { [penNumber]: student });
                }
            }
